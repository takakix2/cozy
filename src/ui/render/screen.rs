...
fn render_welcome_narrow(f: &mut Frame, area: Rect) {
    ...
    let cyan_b   = Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD);
...
fn render_welcome_wide(f: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from(""),
        Line::from(format!("{:<16}{:<17}{}", "Ctrl+O Open", "Ctrl+S Save", "Ctrl+X Exit")),
        Line::from(format!("{:<16}{:<17}{}", "Ctrl+F Find", "Ctrl+R Replace", "Ctrl+H Help")),
        Line::from(format!("{:<16}{:<17}{}", "Ctrl+Z Undo", "Ctrl+Y Redo",   "Ctrl+J Jump")),
        Line::from(format!("{:<16}{:<17}{}", "Ctrl+B Browse", "Ctrl+G Glide",  "")),
        Line::from(""),
...
fn render_help_narrow(editor: &EditorState, f: &mut Frame, area: Rect) {
    ...
    let lines: Vec<Line> = vec![
        Line::from(Span::styled("cozy Help  (↑↓ scroll)", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))),
        Line::from(""),
        ...
        shortcut_pair("H",   "Scr top", "L",   "Scr bot", col, 0),
        shortcut_pair("M",   "Scr mid", "+/-", "Nxt/Prv", col, 0),
...
fn render_help_wide(editor: &EditorState, f: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from(""),
        Line::from(Span::styled("Edit mode: just type.  Ctrl+G → Glide (vim-style).", gray)),
        ...
        Line::from("  F2              Toggle Markdown preview"),
        Line::from(""),
        ...
        Line::from("  H / M / L       Screen top / mid / bottom"),
